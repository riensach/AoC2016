...
$puzzleInput = 'dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
foreach($puzzleRows as $rowID => $rowValues) {
    ...
    //echo "<br>";
    $splitData = NULL;
    preg_match_all("/(?|([a-z]+)+|(\[[a-z]+\])+)/i", $rowValues, $splitData);
    
    $matches = $splitData[0]; // Why is it creating multiple arrays? Because my RegEx sux probably
    $valid = $invalid = 0;
    $possibleOptionsOutside = array();
    $possibleOptionsInside = array();
    foreach($matches as $matchID => $matchData) {
        
        $charListOutside = new SplDoublyLinkedList();
        $charListInside = new SplDoublyLinkedList();
        
        if(substr($matchData, 0, 1)=='[' && substr($matchData, -1, 1)==']') {
            // It's inside the blocks
            $matchData = str_replace("[","",$matchData);
            $matchData = str_replace("]","",$matchData);
            $characters = str_split($matchData,1);
            foreach($characters as $charID => $char) {                
                $charListInside->push($char);
                if($charID>2) {
                        $charListInside->shift();
                }
                if($charID>1 && $charListInside->offsetGet(0)==$charListInside->offsetGet(2) && $charListInside->offsetGet(1)<>$charListInside->offsetGet(2)) {
                    $possibleOptionsInside[] = array('char1' => $charListInside->offsetGet(0),'char2' => $charListInside->offsetGet(1),'char3' => $charListInside->offsetGet(2));
                    $invalid = 1;
                }
            }
        } else {
            ...
            $characters = str_split($matchData,1);
            foreach($characters as $charID => $char) {                
                $charListOutside->push($char);
                if($charID>2) {
                        $charListOutside->shift();
                }
                if($charID>1 && $charListOutside->offsetGet(0)==$charListOutside->offsetGet(2) && $charListOutside->offsetGet(1)<>$charListOutside->offsetGet(2)) {                    
                    $possibleOptionsOutside[] = array('char1' => $charListOutside->offsetGet(0),'char2' => $charListOutside->offsetGet(1),'char3' => $charListOutside->offsetGet(2));
                    $valid = 1;
                }
            }
        }
    ...
    if($valid==1 && $invalid==1) {
        ...
        foreach($possibleOptionsOutside as $id => $value) {
            foreach($possibleOptionsInside as $id2 => $value2) {
                if($possibleOptionsOutside[$id]['char1']==$possibleOptionsInside[$id2]['char2'] && $possibleOptionsOutside[$id]['char2']==$possibleOptionsInside[$id2]['char1']) {
                    echo "valid match: ".$possibleOptionsOutside[$id]['char1'].$possibleOptionsOutside[$id]['char2'].$possibleOptionsOutside[$id]['char3']."-".$possibleOptionsInside[$id2]['char1'].$possibleOptionsInside[$id2]['char2'].$possibleOptionsInside[$id2]['char3']."<br>";
                    $foundMatches++;
                    break 2;
                }
            }
       ...
       // echo "Found a valid match<br>";
    }
    //echo "<br>".print_r($matches)."<br> - $valid - $invalid<br>";
    

    //echo "<br>";
}

echo "<br>Finished. Found a total of $foundMatches valid matches.<br>";
