...
$puzzleInput = 'bot 49 gives low to bot 118 and high to bot 182
...
function botIDCheck($botID,&$maximumBotID,&$lowestBotID) {
    if($botID > $maximumBotID) $maximumBotID = $botID;
    if($botID < $lowestBotID) $lowestBotID = $botID;
}
function findCharacterID($products, $field, $value)
{
...
/*$puzzleInput = 'value 5 goes to bot 2
...
$botArray = array();
$outputArray = array();
foreach($puzzleRows as $row => $value) {
    $values = explode(" ", $value);
    if($values[0]=='bot') {
        $action = 'output';
        $hostBotID = $values[1];
        $lowID = $values[6];
        $lowIDDestinationType = $values[5];
        $highID = $values[11];
        $highIDDestinationType = $values[10];
        botIDCheck($hostBotID,$maximumBotID,$lowestBotID);
        botIDCheck($lowID,$maximumBotID,$lowestBotID);
        botIDCheck($highID,$maximumBotID,$lowestBotID);
        $instructions[] = array('hostBotID' => $hostBotID, 'lowID' => $lowID, 'lowIDDestinationType' => $lowIDDestinationType, 'highID' => $highID, 'highIDDestinationType' => $highIDDestinationType);
    } else {
        $action = 'assign';
        $actionValue = $values[1];
        $hostBotID = $values[5];
        botIDCheck($hostBotID,$maximumBotID,$lowestBotID);
        $assignments[] = array('value' => $actionValue, 'hostBotID' => $hostBotID);

    }
}

for($i=$lowestBotID;$i <= $maximumBotID;$i++) {
    $botArray[$i] = array('value1' => -1, 'value2' => -1);
}

for($i=0;$i <= 300;$i++) {
    $outputArray[$i] = array('value' => -1);
}

foreach($assignments as $id => $value) {
    
    if($botArray[$value['hostBotID']]['value1']<0) {
        $botArray[$value['hostBotID']]['value1'] = $value['value'];
    } elseif($botArray[$value['hostBotID']]['value2']<0) {
        $botArray[$value['hostBotID']]['value2'] = $value['value'];        
    } else {
        echo "Problem.<br>";
    }   
...
$foundReadyBot = 1;
while($foundReadyBot > 0) {
    $foundReadyBot = 0;
    foreach($botArray as $id => $value) {

        if($value['value1'] <0 || $value['value2'] <0) continue;
        $foundReadyBot = 1;
        if(($value['value1'] ==61 || $value['value1'] ==17) && ($value['value2'] ==61 || $value['value2'] ==17) && $value['value1'] <> $value['value2']) {
            echo "First bot to compare chip 61 & 17 is Bot ID $id<br>";
        }
        $instructionID = findCharacterID($instructions,'hostBotID',$id);
        $lowDestType = $instructions[$instructionID]['lowIDDestinationType'];
        $highDestType = $instructions[$instructionID]['highIDDestinationType'];
        $lowDestID = $instructions[$instructionID]['lowID'];
        $highDestID = $instructions[$instructionID]['highID'];
        $lowValue = $value['value1'];
        $highValue = $value['value2'];
        if($value['value1'] > $value['value2']) {
            $lowValue = $value['value2'];
            $highValue = $value['value1'];
        }
        if($lowDestType=='bot') {
            if($botArray[$lowDestID]['value1']<0) {
                $botArray[$lowDestID]['value1'] = $lowValue;
            } elseif($botArray[$lowDestID]['value2']<0) {
                $botArray[$lowDestID]['value2'] = $lowValue;        
            } else {
                echo "Problem.<br>";
            } 
        } else {
            $outputArray[$lowDestID] = $lowValue;
        }

        if($highDestType=='bot') {
            if($botArray[$highDestID]['value1']<0) {
                $botArray[$highDestID]['value1'] = $highValue;
            } elseif($botArray[$highDestID]['value2']<0) {
                $botArray[$highDestID]['value2'] = $highValue;        
            } else {
                echo "Problem.<br>";
            }
        } else {
            $outputArray[$highDestID] = $highValue;
        }

        $botArray[$id]['value1'] = -1;
        $botArray[$id]['value2'] = -1;
    }
}
...
$bCalc = $outputArray[0] * $outputArray[1] * $outputArray[2];


echo "Part B answer: $bCalc<br>";
