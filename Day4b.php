...
$puzzleInput = 'aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
...
/*$puzzleInput = 'aaaaa-bbb-z-y-x-123[abxyz]
...
//$puzzleInput = 'qzmt-zixmtkozy-ivhz-343[zimth]';
...
$puzzleRows = explode("\n",$puzzleInput);
$finalSum = 0;
foreach($puzzleRows as $key => $inputValues) {
    $validRoom = 0;
    $sections = explode("-",$inputValues);
    $lastKey = array_key_last($sections);    
    $finalData = explode("[",$sections[$lastKey]);
    $sectorID = $finalData[0];
    $checkSum = str_replace("]", "", $finalData[1]);
    $i = 0;
    $string = '';
    ...
    while($i<count($sections)-1) {
        $string .= $sections[$i];
        $roomNameString .= ' '.$sections[$i];
        $i++;
    }
    $roomNameString = ltrim($roomNameString);
    //$string = str_replace(" ", "", $string);
    $foundCharacters = count_chars($string, 1);
    $foundCharactersArray = array();
    foreach($foundCharacters as $charID => $countFound) {
        $foundCharactersArray[] = array('charID' => $charID,'foundCount' => $countFound);
    }
     
   
    array_multisort(array_column($foundCharactersArray, 'foundCount'), SORT_DESC,
                    array_column($foundCharactersArray, 'charID'), SORT_ASC,
                    $foundCharactersArray);

    $compareString = '';
    $i = 0;
    while ($i<5) {
        $compareString .= chr($foundCharactersArray[$i]['charID']); 
        $i++;
    }    
    if($compareString == $checkSum) {
        ...
        // Calculate room name
        $roomNameStringSplit = str_split($roomNameString,1);
        $roomNameStringCompuited = '';
        foreach ($roomNameStringSplit as $id => $character) {
            if($character==" ") {
                $roomNameStringCompuited .= " ";
                continue;
            }
            //print_r($alphabet);
            $currentArrayKey = array_search($character, $alphabet);
            $addition = $sectorID % 26;
            $newKey = $currentArrayKey + $addition;
            //echo "$character-$currentArrayKey-$sectorID-$addition-$newKey<br>";
            if($newKey>25) {
                $newKey = $newKey - 26;
            }
            $roomNameStringCompuited .= $alphabet[$newKey];
        }

        if($roomNameStringCompuited == 'northpole object storage') {
            $targetSectorID=$sectorID;
        }
        //echo "$string - $sectorID - $checkSum - $compareString - $roomNameStringCompuited :: real room<br>";
    } else {
        //echo "NOIT $string - $sectorID - $checkSum - $compareString ::<br>";
    }
    //die();
}
...
echo "Part 1: $finalSum";
echo "<br>Part 2: Sectolr ID : $targetSectorID";
